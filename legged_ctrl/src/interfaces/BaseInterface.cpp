...
namespace legged {
    ...
    bool BaseInterface::joy_update(double dt) {
        ...
        Eigen::Vector3d joy_vel_cmd_world = legged_state.fbk.torso_rot_mat_z * joy_vel_cmd;
        legged_state.joy.body_x += joy_vel_cmd_world[0] * dt;
        legged_state.joy.body_y += joy_vel_cmd_world[1] * dt;
        legged_state.joy.body_height += joy_vel_cmd_world[2] * dt;
    ...
    bool BaseInterface::sensor_update(double t, double dt) {
        ...
        // std::cout << "torso_rot_mat: " << std::endl << legged_state.fbk.torso_rot_mat << std::endl;
        // std::cout << "torso_ang_vel_body: " << legged_state.fbk.torso_ang_vel_body << std::endl;
        // std::cout << "torso_ang_vel_world: " << legged_state.fbk.torso_ang_vel_world.transpose() << std::endl;
        ...
        for (int i = 0; i < NUM_LEG; ++i) {
            ...
            legged_state.fbk.foot_pos_com(0, i) = legged_state.fbk.foot_pos_body(0, i) - torso_com_x_body;
            legged_state.fbk.foot_pos_com(1, i) = legged_state.fbk.foot_pos_body(1, i) - torso_com_y_body;
            legged_state.fbk.foot_pos_com(2, i) = legged_state.fbk.foot_pos_body(2, i) - torso_com_z_body;
        ...
        // Spider man
        legged_state.ctrl.foot_pos_target_abs = legged_state.fbk.torso_rot_mat * legged_state.param.default_foot_pos_rel;        
    ...
    bool BaseInterface::tau_ctrl_update(double dt) {
        for (int i = 0; i < NUM_LEG; ++i) {
            ...
            if (legged_state.ctrl.movement_mode > 0) {
                ...
                // std::cout << "Joint ang tgt: " << legged_state.ctrl.joint_ang_tgt.segment<3>(i * 3) << std::endl;
                ...
                // std::cout << "Joint vel tgt: " << legged_state.ctrl.joint_vel_tgt.segment<3>(i * 3) << std::endl;
                ...
                if (legged_state.ctrl.plan_contacts[i]) {
                    ...
                    if (i == 0) {
                        go1_dyn.go1_FL_inverse_dynamics_task_space(legged_state.ctrl.joint_ang_tgt.segment<3>(i * 3),
                                                                   foot_vel_target_body.block<3, 1>(0, i), foot_acc_target_body.block<3, 1>(0, i),
                                                                   tau);
                    } else if (i == 1) {
                        go1_dyn.go1_FR_inverse_dynamics_task_space(legged_state.ctrl.joint_ang_tgt.segment<3>(i * 3),
                                                                   foot_vel_target_body.block<3, 1>(0, i), foot_acc_target_body.block<3, 1>(0, i),
                                                                   tau);
                    } else if (i == 2) {
                        go1_dyn.go1_RL_inverse_dynamics_task_space(legged_state.ctrl.joint_ang_tgt.segment<3>(i * 3),
                                                                   foot_vel_target_body.block<3, 1>(0, i), foot_acc_target_body.block<3, 1>(0, i),
                                                                   tau);
                    } else if (i == 3) {
                        go1_dyn.go1_RR_inverse_dynamics_task_space(legged_state.ctrl.joint_ang_tgt.segment<3>(i * 3),
                                                                   foot_vel_target_body.block<3, 1>(0, i), foot_acc_target_body.block<3, 1>(0, i),
                                                                   tau);
                    }
                    legged_state.ctrl.joint_tau_tgt.segment<3>(i * 3) = tau + 0.2 * legged_state.ctrl.joint_vel_tgt.segment<3>(i * 3);
                    // std::cout << "Joint tau tgt: " << legged_state.ctrl.joint_tau_tgt.segment<3>(i * 3).transpose() << std::endl;
                }
