...
namespace legged {
    ...
    bool ConvexMpc::goal_update(LeggedState &state) {
        ...
        state.ctrl.torso_pos_d[0] = state.joy.body_x;
        state.ctrl.torso_pos_d[1] = state.joy.body_y;       
    ...
    bool ConvexMpc::grf_update(LeggedState &state) {
        ...
        for (int i = 0; i <= horizon; i++) {
            x_ << 0.0, // roll, always 0
                  ...
                  state.ctrl.torso_pos_d[0], // x, don't track
                  state.ctrl.torso_pos_d[1], // y, don't track
                  state.ctrl.torso_pos_d[2],  // z
    ...
    bool ConvexMpc::terrain_update(LeggedState &state) {
        ...
        if (state.ctrl.movement_mode > 0 && state.fbk.torso_pos_world[2] > 0.1) {
            ...
            terrain_angle = terrain_angle_filter.CalculateAverage(
                    Utils::cal_dihedral_angle(flat_ground_coef, walk_surf_coef));
            ...
            double F_R_diff = state.fbk.foot_pos_abs_prev(2, 0) + state.fbk.foot_pos_abs_prev(2, 1) -
                              state.fbk.foot_pos_abs_prev(2, 2) - state.fbk.foot_pos_abs_prev(2, 3); // FL, FR, RL, RR
            if (F_R_diff > 0.05) {
