...
namespace legged {
    ...
    void LeggedContactFSM::reset() {
        ...
        gait_pattern_idx = 0;
        prev_gait_pattern_idx = gait_pattern_size - 1;
        cur_state_start_time = 0;
        cur_state_end_time = gait_pattern_switch_time[gait_pattern_idx];
        ...
        s = gait_pattern[gait_pattern_idx];
        // terrain_height = 0;
    ...
    double LeggedContactFSM::update(double dt, double gait_freq, Eigen::Vector3d foot_pos_cur_world, Eigen::Vector3d foot_pos_target_world, bool foot_force_flag) {
        ...
        if (s == STANCE) {
            if (gait_phase < 1.0) {
                ...
                if (gait_pattern[0] == SWING) {
                    stance_exit(foot_pos_cur_world);
                    swing_enter(foot_pos_cur_world);
                    s = SWING;
                } else if (gait_pattern[0] == STANCE) {
                    stance_exit(foot_pos_cur_world);
    ...
    void LeggedContactFSM::set_trotting_gait() {
        gait_pattern.clear();
        gait_pattern_idx = 0;
        gait_pattern_switch_time.clear();
        if (leg_id == 0 || leg_id == 3) {
            gait_pattern.push_back(STANCE);
            gait_pattern.push_back(SWING);
        } else {
            gait_pattern.push_back(SWING);
            gait_pattern.push_back(STANCE);
        }
        gait_pattern_switch_time.push_back(0.5);
        gait_pattern_switch_time.push_back(1.0);
    ...
    void LeggedContactFSM::set_static_gait() {
        gait_pattern.clear();
        gait_pattern_idx = 0;
        gait_pattern_switch_time.clear();
        if (leg_id == 0) {
            gait_pattern.push_back(SWING);
            gait_pattern.push_back(STANCE);
            gait_pattern_switch_time.push_back(0.25);
            gait_pattern_switch_time.push_back(1.0);
            gait_pattern_size = 2;
        } else if (leg_id == 1) {
            gait_pattern.push_back(STANCE);
            ...
            gait_pattern.push_back(STANCE);
            gait_pattern_switch_time.push_back(0.5);
            gait_pattern_switch_time.push_back(0.75);
            gait_pattern_switch_time.push_back(1.0);
            gait_pattern_size = 3;
        } else if (leg_id == 3) {
            gait_pattern.push_back(STANCE);
    ...
    void LeggedContactFSM::set_climb_gait() {
        gait_pattern.clear();
        gait_pattern_idx = 0;
        gait_pattern_switch_time.clear();
        if (leg_id == 0) {
    ...
    void LeggedContactFSM::set_trot_with_stand_gait_pattern() {
        ...
        if (leg_id == 0) {            //FL
            gait_pattern.push_back(STANCE);
            gait_pattern.push_back(SWING);
            gait_pattern_switch_time.push_back(0.6);
            gait_pattern_switch_time.push_back(1.0);
            gait_pattern_size = 2;
        } else if (leg_id == 1) {     //FR
            gait_pattern.push_back(STANCE);
            gait_pattern.push_back(SWING);
            gait_pattern.push_back(STANCE);
            gait_pattern_switch_time.push_back(0.1);
            gait_pattern_switch_time.push_back(0.5);
            gait_pattern_switch_time.push_back(1.0);
            gait_pattern_size = 3;
        } else if (leg_id == 2) {     //RL
    ...
    void LeggedContactFSM::set_default_stand_pattern() {
        ...
        gait_pattern_size = 1;
        gait_pattern_idx = 0;
        prev_gait_pattern_idx = 0;
        gait_pattern_loaded = true;
    }

    void LeggedContactFSM::common_enter() {
        // first increase pattern index
        prev_gait_pattern_idx = gait_pattern_idx;
        gait_pattern_idx = (gait_pattern_idx + 1) % gait_pattern_size;
        if (gait_pattern_idx < prev_gait_pattern_idx) {
            // warp gait phase 
            gait_phase -= 1.0;
        }
...
//       }
    ...
    LeggedContactState LeggedContactFSM::predict_contact_state(double dt) {
        ...
        for (int i = 0; i < gait_pattern_size; i++) {
            if (predicted_gait_phase <= gait_pattern_switch_time[i]) {
                return gait_pattern[i];
            }
        }
