...
// THIS FUNCTION SHOULD BE GOOD
void QuadrupedModel::ct_srb_quat_dynamics(double *x_dot, const double *x, const double *u,
                                          Eigen::Matrix<double, 3, 4> foot_pos_body, Eigen::Matrix3d inertia_body, Eigen::Matrix3d torso_rot_mat) const {
    Eigen::Map<Eigen::VectorXd> x_dot_vec(x_dot, 13);
    Eigen::Map<const Eigen::VectorXd> x_vec(x, 13);
    Eigen::Map<const Eigen::VectorXd> u_vec(u, 12);

    double robot_mass = 12.84;
    Eigen::Vector3d g_vec_abs;
    Eigen::Vector3d g_vec_body;
    g_vec_abs << 0, 0, -9.81;
    g_vec_body = torso_rot_mat.transpose() * g_vec_abs;
    ...
    x_dot_vec.segment<3>(10) = inertia_body.inverse() * (moment_body - x_vec.segment<3>(10).cross(inertia_body * x_vec.segment<3>(10)));
}

// THIS FUNCTION SHOULD BE GOOD
void QuadrupedModel::ct_srb_quat_jacobian(double *jac, const double *x, const double *u, Eigen::Matrix<double, 3, 4> foot_pos_body, Eigen::Matrix3d inertia_body) const {
    (void)u;
    ...
    // domegadot/domega
    dfc_dx.block<3, 3>(10, 10) = inertia_body.inverse() * (Utils::skew(x_vec.segment<3>(10)) * inertia_body - Utils::skew(inertia_body * x_vec.segment<3>(10)));
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        dfc_du.block<3, 3>(10, 3 * i) = inertia_body.inverse() * Utils::skew(foot_pos_body.block<3, 1>(0, i));
    }
